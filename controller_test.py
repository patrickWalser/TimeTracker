...
class TimeTrackerUnitTest(unittest.TestCase):
    ...
    def test_timer_updates_status(self):
        ...
        self.assertEqual(
            len(updates), 4, "observer was not called 3 times within 3 seconds")

        for i in range(len(updates)):
            self.assertIsInstance(
                updates[i], datetime.timedelta, "observer did not pass correct datatype as parameter")
            if (i < len(updates) - 1):
    ...
    def test_get_object_by_id(self):
        ...
        semester, module, entry = timeTracker._study.add_entry(
            "sem", "mod", "cat", "com")
    ...
    def test_finish_module(self):
        ...
        self.assertAlmostEquals(
            mod.stop, time, delta=datetime.timedelta(seconds=1))
    ...
    def test_add_new_entry(self):
        ...
        sem, mod, entry = timeTracker.add_new_entry(
            semester_name, module_name, category, comment, start_time, stop_time)
    ...
    def test_edit_entry(self):
        ...
        timeTracker.edit_entry(semester, module, entry, edit_semester_name, edit_module_name, edit_category, edit_comment,
                               edit_start_time, edit_stop_time, edit_module_start, edit_module_stop, edit_ects, edit_duration)
        ...
        self.assertEqual(new_module.plannedEnd,
                         edit_module_start + datetime.timedelta(weeks=3))
    ...
    def test_get_burndown_chart_data_study(self):
        ...
        # Create mock data
        semester1 = model.Semester(
            name="sem1", ECTS=10, plannedEnd=datetime.datetime(2023, 12, 31))
        semester2 = model.Semester(
            name="sem2", ECTS=10, plannedEnd=datetime.datetime(2024, 12, 31))
        ...
        stop_times, values, total_work, planned_end = timeTracker._get_burndown_chart_data(
            self.study)

        # Assertions
        self.assertEqual(total_work, 180)
        self.assertEqual(planned_end, datetime.datetime(2024, 12, 31))
        self.assertEqual(stop_times, [datetime.datetime(2023, 1, 1), datetime.datetime(2023, 6, 30), datetime.datetime(
            2023, 12, 31), datetime.datetime(2024, 6, 30), datetime.datetime(2024, 12, 31)])
    ...
    def test_get_burndown_chart_data_semester(self):
        ...
        semester = model.Semester(
            name="sem", ECTS=10, plannedEnd=datetime.datetime(2023, 12, 31))
        ...
        self.assertEqual(stop_times, [datetime.datetime(2023, 1, 1), datetime.datetime(
            2023, 6, 30), datetime.datetime(2023, 12, 31)])
    ...
    def test_import_from_json(self):
        ...
        new_timeTracker = controller.TimeTracker(model.Study(
            ECTS=0, hoursPerECTS=0, plannedEnd=datetime.datetime.now()))
        ...
        self.assertEqual(new_timeTracker._study.hoursPerECTS,
                         self.study.hoursPerECTS)
    ...
    def test_update_semester(self):
        ...
        semester = model.Semester(
            name="sem", ECTS=30, plannedEnd=datetime.datetime(2023, 12, 31))
        self.study.semesters.append(semester)
        ...
        for date in valid_dates:
            updated_semester = timeTracker.update_semester(
                semester, "plannedEnd", date)
            self.assertEqual(updated_semester.plannedEnd,
    ...
    def test_get_module_names(self):
        ...
        # Test with non-existent semester
        non_existent_module_names = timeTracker.get_module_names(
            "non_existent_sem")
    ...
    def test_get_category_names(self):
        ...
        category_names = timeTracker.get_category_names(
        ...
        # Test with non-existent semester
        non_existent_categories = timeTracker.get_category_names(
            "non_existent_sem", module_name)
        self.assertEqual(non_existent_categories, [])

        # Test with non-existent module
        non_existent_categories = timeTracker.get_category_names(
            semester_name, "non_existent_mod")
        self.assertEqual(non_existent_categories, [])
    ...
    def test_create_new_study(self):
        ...
        # Create a new study
        new_study = timeTracker.create_new_study(
            new_ECTS, new_hoursPerECTS, new_plannedEnd)
    ...
    def test_get_last_tracking_information(self):
        ...
        last_info = timeTracker.get_last_tracking_information()
        self.assertEqual(last_info, (semester.name, module.name,
                         entry.category, entry.comment))
